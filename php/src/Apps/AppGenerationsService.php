...
final class AppGenerationsService
{
     ...
     */
    public function create(string $appId, array $parameters): array
    {
        return $this->transport->request(
            'POST',
            '/api/v1/apps/' . rawurlencode($appId) . '/generations',
            $parameters,
        );
    }
     ...
     * @return array<string, mixed>
     */
    public function retrieve(string $appId, string $generationId): array
    {
        return $this->transport->request(
            'GET',
            '/api/v1/apps/' . rawurlencode($appId) . '/generations/' . rawurlencode($generationId),
        );
    }
     ...
     * @return array<string, mixed>
     */
    public function wait(string $appId, string $generationId, ?WaitOptions $options = null): array
    {
        $options ??= new WaitOptions();
        $deadline = microtime(true) + $options->timeout;

        while (true) {
            $envelope = $this->retrieve($appId, $generationId);
            ...
            if (microtime(true) >= $deadline) {
                throw new TimeoutException(
                    "Timed out waiting for Buble app generation '{$generationId}'.",
                    $options->timeout,
                );
            }
