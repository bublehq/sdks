...
| `buble` | `cargo add buble` | Rust 1.88+ | `rust/` | [Rust README](rust/README.md), [crates.io](https://crates.io/crates/buble), [docs.rs](https://docs.rs/buble) after publication |
...
The Rust SDK also reads `BUBLE_API_KEY` and `BUBLE_BASE_URL` from the environment when omitted. API documentation is generated by rustdoc and published to [docs.rs](https://docs.rs/buble) after the crate is published to [crates.io](https://crates.io/crates/buble).
...
- Confirm `buble` is still available on crates.io.
- Create or sign in to the Buble crates.io account.
- Add the Buble crates.io account as an owner for the `buble` crate after the first publish, if the first publish is performed by an individual maintainer.
